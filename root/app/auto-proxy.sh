...
for DOCKER_HOST in ${DOCKER_HOSTS}; do
    ...
    if [ ! -f ${ENABLED_FILE} ]; then
        ...
        EXISTING_CONFS=$(cat ${ENABLED_FILE})
        ...
        echo "${ENABLED_CONTAINERS}" > ${ENABLED_FILE}
    fi

    for CONTAINER in ${AUTO_GEN}; do
        GENERATED_CONTAINERS="true"
        INSPECTION=$(docker inspect ${CONTAINER})
...
if ([ "${GENERATED_CONTAINERS}" == "true" ] || [ "${REMOVED_CONTAINERS}" == "true" ]) && ps aux | grep [n]ginx: > /dev/null; then 
